...
mcmen_tract = data.frame(pnw_tracts[mcmen_loc, ])

#transform mcmen tracts to spatial
mcmen_tract_shape = as(mcmen_tract, "Spatial")
```
...
tract_table = tract_table %>% 
  select(-contains(c("tract.x.x", "tract.y"))) %>% 
  select(-c(`tract`))%>% 
...
#transform urban areas to spatial
urban_shape = as(urban_shape, "Spatial")
